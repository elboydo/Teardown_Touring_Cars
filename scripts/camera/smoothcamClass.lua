...
function smoothcam:init()
	...
	for i = 2, #smoothcams-1, 2 do
		bezierPoints[#bezierPoints+1] = self:bezierCenter(smoothcams[i-1].pos, smoothcams[i].pos)
		bezierPoints[#bezierPoints+1] = smoothcams[i].pos
		bezierPoints[#bezierPoints+1] = smoothcams[i+1].pos

		if i + 2 <= #smoothcams then
			bezierPoints[#bezierPoints+1] = self:bezierCenter(smoothcams[i+1].pos, smoothcams[i+2].pos)
		end
	end
end

function smoothcam:tick(dt)
...
function smoothcam:cubicBezier(ps,t)
...
function smoothcam:cubicBezierPoint(a0,a1,a2,a3,t)
	return math.pow(1 - t, 3) * a0 + 3 * math.pow(1 - t, 2) * t * a1 + 3 * (1 - t) * math.pow(t, 2) * a2 + math.pow(t, 3) * a3;
end

function smoothcam:bezierCenter(a,b)
	local x = (a[1] + b[1]) / 2
	local y = (a[2] + b[2]) / 2
	local z = (a[3] + b[3]) / 2
	local v = Vec(x,y,z)
